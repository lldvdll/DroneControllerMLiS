...
class Reinforce1LayerController(FlightController):
    ...
    def __init__(self):
        ...
        # Set learning rate
        self.filename = "reinforce_1layer_weights.npz"
    ...
    def train(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(max_steps):
                ...
                r_ddist = (dist0 - r_dist) * (not r_hit) * self.rewards['ddist']  # Change in distance reward. Positive means closer. 0 if we hit to avoid teleporting cost
                r_ddist = np.clip(r_ddist, -1.0, 1.0)
            ...
            # Save periodically
            if episode % 100 == 0:
                self.save(filename="reinforce_1layer_weights_latest")
